...
<form action="/gestionecole/classe/edit?id=<?= $classe['id'] ?>" method="POST">
    ...
    <div>
        ...
        <select id="nom" name="nom" required>
            ...
            <?php foreach (['CP1', 'CP2', 'CE1', 'CE2', 'CM1', 'CM2'] as $niveau): ?>
                <option
                    value="<?= $niveau ?>"
                    <?= ($_POST['nom'] ?? $classe['nom']) === $niveau ? 'selected' : '' ?>
                >
                    <?= $niveau ?>
                </option>
            <?php endforeach; ?>
    ...
    <div>
        ...
        <select id="enseignant_id" name="enseignant_id">
            ...
            <?php foreach ($enseignants as $enseignant): ?>
                <option
                    value="<?= $enseignant['id'] ?>"
                    <?= ($_POST['enseignant_id'] ?? $classe['enseignant_id']) == $enseignant['id'] ? 'selected' : '' ?>
                >
